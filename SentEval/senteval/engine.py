...
class SE(object):
    def __init__(self, params, batcher, prepare=None, mode='test'):
        # parameters
        params = utils.dotdict(params)
        params.usepytorch = True if 'usepytorch' not in params else params.usepytorch
        params.seed = 1111 if 'seed' not in params else params.seed

        params.batch_size = 256 if 'batch_size' not in params else params.batch_size
    ...
    def eval(self, name):
        ...
        if name == 'CR':
            ...
            self.evaluation = SICKRelatednessEval(tpath + '/downstream/SICK', seed=self.params.seed, mode=self.mode)
        elif name == 'STSBenchmark':
            self.evaluation = STSBenchmarkEval(tpath + '/downstream/STS/STSBenchmark', seed=self.params.seed, mode=self.mode)
