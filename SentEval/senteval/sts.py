...
class STSEval(object):
    ...
    def do_prepare(self, params, prepare):
        ...
        self.align = lambda s1, s2: (s1-s2).norm(dim=1).pow(2).mean()
        self.unifm = lambda s: torch.pdist(s, p=2).pow(2).mul(-2).exp().mean().log()
    ...
    def run(self, params, batcher):
        ...
        logging.debug('ALL : Pearson = %.4f, Spearman = %.4f\n' % (all_pearson[0], all_spearman[0]))
...
class STSBenchmarkEval(STSEval):
    def __init__(self, task_path, seed=1111, mode='test'):
